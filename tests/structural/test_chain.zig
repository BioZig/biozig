...
test "Chain large payload and bounds checks" {
    ...
    for (0..size) |i| {
        // synthetic residues
        const synthetic_atoms = try alloc.alloc(atom.Atom, 0); // we can use empty residues
        defer alloc.free(synthetic_atoms);
        large_residues[i] = try residue.Residue.init(i, "UNK", synthetic_atoms);
    }
