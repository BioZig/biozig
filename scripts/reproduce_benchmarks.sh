...
if [ $COMPREHENSIVE -eq 1 ]; then
    ...
    echo -e "\n${CYAN}[*] Target 1: Full GRCh38 Pan-Genome Telemetry (Markov Transitions)${NC}"
    /usr/bin/time $TIME_FLAG ./zig-out/bin/biozig fetch --db ncbi --query GRCh38 --analyze markov 2>&1 | grep -iE "resident set size|elapsed|user" || true

    echo -e "\n${CYAN}[*] Target 2: Full GRCh38 FASTQ/Sequence Statistics${NC}"
    /usr/bin/time $TIME_FLAG ./zig-out/bin/biozig fetch --db ncbi --query GRCh38 --analyze fastq_stats 2>&1 | grep -iE "resident set size|elapsed|user" || true

    echo -e "\n${CYAN}[*] Target 3: Structural Alignment (PCA - 6vxx PDB API Stream)${NC}"
    /usr/bin/time $TIME_FLAG ./zig-out/bin/biozig fetch --db pdb --query 6vxx --analyze pca 2>&1 | grep -iE "resident set size|elapsed|user" || true

    echo -e "\n${RED}[!] ATTENTION REVIEWER: Targets 4 & 5 require local dataset downloads.${NC}"
    echo -e "${RED}[!] If string_db_9606.txt and gse100866.mtx are missing, these will safely terminate at 1.6 MB RSS.${NC}"

    echo -e "\n${CYAN}[*] Target 4: PPI Network Topology Analysis (Requires Local STRING-DB)${NC}"
    /usr/bin/time $TIME_FLAG ./zig-out/bin/biozig systems network -i string_db_9606.txt 2>&1 | grep -iE "resident set size|elapsed|user" || true
